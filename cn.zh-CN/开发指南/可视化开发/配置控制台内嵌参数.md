...
|savedsearchID|string|是|快速查询ID。**说明：** 您在创建快速查询后，可以在URL中获取快速查询ID。更多信息，请参见[获取快速查询ID](/cn.zh-CN/查询与分析/查询语法与功能/快速查询.md)。
...
|dashboardID|string|是|仪表盘ID。**说明：** 您在创建仪表盘后，可以在URL中获取仪表盘ID。更多信息，请参见[接口说明](/cn.zh-CN/开发指南/API参考/仪表盘接口/ListDashboard.md)。
