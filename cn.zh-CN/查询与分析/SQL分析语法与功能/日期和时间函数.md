...
|`from_unixtime(unixtime,string)`|以string为时区，把unixtime转化为日期时间。|`latency>100| select from_unixtime (1494985275,'Asia/Shanghai')`|
...
-   参数说明：
    ...
    x可以是一个timestamp类型，也可以是unix time。
...
-   示例：
    ...
    * | select time_series(__time__, '2h', '%Y-%m-%d %H:%i:%s', '0')  as stamp, count(*) as num from log group by stamp order by stamp
    ...
    ![时间补全函数示例图](https://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/7140559951/p37530.png)
