...
-   参数说明
    ...
    |mode|String|否|字段的覆盖模式。默认为fill-auto。关于更多字段值及含义参见[字段提取检查与覆盖模式](/cn.zh-CN/数据加工/数据加工语法/通用参考/字段提取模式.md)。|
...
-   参数说明
    ...
    |参数名称|参数类型|是否必填|说明|
    |----|----|----|--|
    |data|Dict|是|映射字典，标准的关键字/值的配对，要求关键字必须是搜索字符串。关于搜索字符串具体请参见[dct\_get](/cn.zh-CN/数据加工/数据加工语法/表达式函数/字典函数.md)函数。|
    |output\_field|String|是|输出字段的名称。|
