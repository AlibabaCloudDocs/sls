...
-   函数示例
    ...
    """星号（*）匹配零个或多个, 问号（?）匹配一个字符"""
    e_search("status: active*test")    # 星号（*）可以不用双引号。
    e_search("status: active?good")    # 问号（?）可以不用双引号。
    e_search("status== ac*tive?good")  # 完全匹配。
    
    """值转义 ( *, ? 在值中需要转义 )"""
    e_search('status: "\*\?()[]:="')  # 包含特殊字符的值，需要使用双引号（""）包裹,除了星号（*）、问号（?）和反斜线（\）需要转义外，其他不用转义。
    e_search("status: active\*test")  # 值仅包含星号（*）或问号（?），可以不用双引号。
    e_search("status: active\?test")  # 值仅包含星号（*）或问号（?），可以不用双引号。
    
    """字段名转义"""
    e_search("\*\(1+1\)\?: abc")                 # 字段不能用双引号（""）包裹，特殊字符用反斜线（\）转义。
