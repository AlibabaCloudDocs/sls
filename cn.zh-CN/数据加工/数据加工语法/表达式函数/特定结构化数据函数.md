...
-   参数说明
    ...
    |----|----|----|--|
    |ip|IP字符串|是|IP字符串。|
    |ip\_db|IP库内容|是|唯一IP库，当前使用`res_oss_file(endpoint, ak_id, ak_key, bucket, file, format='text', change_detect_interval=0,fetch_interval=2,refresh_retry_max=60,encoding='utf8',error='ignore')`，参数定义请参见[res\_oss\_file](/cn.zh-CN/数据加工/数据加工语法/表达式函数/资源函数.md)，该函数获取IP库使用的形式为`format='binary'`。|
...
此外`keep_fields`也支持重命名。例如`(("city","cty"),("country","state"))`表示以`cty`和`state`形式输出。 |
    |provider|字符串|否|取值如下：     -   ipip：默认值。用来解析ipdb库提供的二进制IP地址库，下载地址为[ipip](https://www.ipip.net/?origin=EN)。
...
-   参数说明
    ...
    |defalut|任意|否|设置default的值（可为空），无法将IP地址转成CIDR地址块时，返回该值。|
