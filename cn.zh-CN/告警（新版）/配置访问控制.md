...
4.  为RAM角色授予监控告警权限策略。
    ...
    3.  在**自定义策略**页签，选中[步骤2](#step_pzo_yvb_cmr)中创建的策略（例如sls-alert-monitor-1-policy），单击**确定**。
...
4.  为RAM角色授予监控告警权限策略。
    ...
    3.  在**自定义策略**页签，选中[2](#step_8ln_m0o_e8e)中创建的策略（例如sls-alert-monitor-2-policy），单击**确定**。
