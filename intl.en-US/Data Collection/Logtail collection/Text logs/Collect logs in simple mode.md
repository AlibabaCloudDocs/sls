...
When you collect logs in simple mode, logs are not parsed. Each log entry is collected and sent to Log Service as a whole. This simplifies the process of log collection. This topic describes how to configure Logtail in simple mode in the Log Service console to collect logs.
...
The simple mode includes the single-line mode and multi-line mode.

-   In single-line mode, each log line is collected as a log entry. Two log entries in a log file are separated by a line feed.
-   In multi-line mode, multiple log lines are collected as a log entry by default.

In single-line mode, you must specify the directories and names of log files. Then, Logtail collects logs by line from the specified files. In multi-line mode, you must specify the directories and names of log files. In addition, you must enter a sample log entry and configure a regular expression to match the first line of a log entry. Logtail uses the regular expression to match the first line of a log entry and considers unmatched lines as part of the log entry. If you collect logs in simple mode, the timestamp of a log entry indicates the system time of the server when the log entry is collected.
...
ln -s /home/log/nginx/log /home/log/nginx/link_log
    ...
    |Log Sample|Enter a sample log entry that is retrieved from a log source in an actual scenario. Then, Log Service extracts a regular expression to match the first line of the log entry. Example:    ```
...
If you collect single-line text logs in simple mode, you do not need to set this parameter. |
    |Regex to Match First Line|The regular expression that Logtail uses to match the first line of a log entry. The unmatched lines are collected as part of a log entry. You can specify a regular expression to match the first line of a log entry. You can also use the regular expression that is automatically generated by Log Service.    -   Automatically generate a regular expression to match the first line of a log entry
