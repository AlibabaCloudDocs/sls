...
`logtailConfig`字段的详细说明请参见[Logtail配置](/intl.zh-CN/开发指南/API 参考/公共资源说明/Logtail配置.md)。`logtailConfig`的配置示例请参见[示例（标准输出）](#section_c3g_wm1_f2b)和[示例（文本文件）](#section_qlv_zm1_f2b)。

创建完成后，自动应用该采集配置。

## 查看采集配置

您可以通过CRD或控制台查看采集配置，其中控制台方式请参见[管理Logtail采集配置](/intl.zh-CN/数据采集/Logtail采集/机器组/管理Logtail采集配置.md)。
