...
确认已安装支持状态查询功能的Logtail客户端之后，在客户端输入相对命令即可查询本地采集状态。如何安装Logtail，请参见[安装Logtail（Linux系统）](/intl.zh-CN/数据采集/Logtail采集/安装/安装Logtail（Linux系统）.md)。
...
1.  `send_block_flag`为true，则说明日志采集进度延迟block在网络部分。
    -   若`send_network_quota_count`大于0时，需要对Logstore的Shard进行分裂，具体请参见[管理Shard](/intl.zh-CN/数据采集/准备工作/管理Shard.md)。
...
2.  发送部分相关参数正常但`avg_delay_bytes`较高。
    ...
    -   可适当调整logtail的资源使用限制，具体请参见[配置Logtail启动参数](/intl.zh-CN/数据采集/Logtail采集/安装/配置Logtail启动参数.md#)。
