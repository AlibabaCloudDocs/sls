...
-   全文查询和字段查询
    ...
    |字段查询|配置字段索引后，您可以指定字段名称和字段值（Key:Value）进行查询。根据字段索引中设置的数据类型，您可以进行多种类型的基础查询和组合查询。更多信息，请参见[数据类型](/intl.zh-CN/查询与分析/数据类型.md)。|`request_time>60 and request_method:Ge*`表示查询request\_time字段值大于60且request\_method字段值以Ge开头的日志。|
