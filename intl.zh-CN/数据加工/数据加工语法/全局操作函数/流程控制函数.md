...
-   函数示例
    -   示例1：字段值匹配后再进行操作。
        ...
        字段`result`是`failed`或`failure`时，设置日志主题为`login_failed_event`。
...
-   函数示例：
    -   如果`content`字段的值是123，则将`__topic__`字段的值设置为Number。如果`data`字段的值是123，则将`__topic__`字段的值设置为PRO。
    ...
    -   通过e\_switch语法和e\_output语法结合，将符合规则的日志投递到不同的Logstore。其中default=e\_drop\(\)，表示将不满足规则的日志丢弃，不做投递处理。若不设置default参数，则表示满足规则日志都会被投递到配置的第一个Logstore中。
