...
**说明：** 使用如下资源函数时，必须配置高级预览才能拉取到目标数据。如何配置高级预览，请参见[t1915388.md\#section\_gfe\_hc5\_mzw](/intl.zh-CN/数据加工/预览调试/预览调试概述.md)。
...
-   参数说明
    ...
    |名称|类型|是否必填|说明|
    |--|--|----|--|
    |endpoint|String|是|访问域名。更多信息，请参见[服务入口](/intl.zh-CN/开发指南/API参考/服务入口.md)。默认为HTTPS格式，也支持HTTP格式。特殊情况下，需使用非80、非443端口。|
